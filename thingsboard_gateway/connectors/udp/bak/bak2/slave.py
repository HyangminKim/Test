...
from connectors.udp.bytes_udp_downlink_converter import BytesUDPDownlinkConverter
    ...
    def __load_converters(self, connector, gateway):
        try:
            ...
            if self.config.get(UPLINK_PREFIX + CONVERTER_PARAMETER) is not None:
                ...
                converter = BytesUDPUplinkConverter({**self.config, 'deviceName': self.name})
            ...
            if self.config.get(DOWNLINK_PREFIX + CONVERTER_PARAMETER) is not None:
                ...
                downlink_converter = BytesUDPDownlinkConverter(self.config)
