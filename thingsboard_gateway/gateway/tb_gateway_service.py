...
import gateway.constants as cons
...
import storage.file.file_event_storage as FileEventStorage
import storage.memory.memory_event_storage as MemoryEventStorage
import storage.sqlite.sqlite_event_storage as SQLiteEventStorage

from thingsboard_gateway.tb_utility.tb_gateway_remote_configurator import RemoteConfigurator
# from thingsboard_gateway.tb_utility.tb_loader import TBModuleLoader
import tb_utility.tb_loader as loader
...
class TBGatewayService:
    def __init__(self, config_file=None):
        ...
        self._event_storage_types = {
            "memory": MemoryEventStorage.MemoryEventStorage,
            "file": FileEventStorage.FileEventStorage,
            "sqlite": SQLiteEventStorage.SQLiteEventStorage,
            }
    ...
    def _load_connectors(self):
        ...
        if self.__config.get("connectors"):
            for connector in self.__config['connectors']:
                try:
                    connector_class = loader.TBModuleLoader.import_module(connector["type"],
    ...
    def __load_persistent_devices(self):
        devices = {}
        if cons.CONNECTED_DEVICES_FILENAME in listdir(self._config_dir) and \
                path.getsize(self._config_dir + cons.CONNECTED_DEVICES_FILENAME) > 0:
            try:
                with open(self._config_dir + cons.CONNECTED_DEVICES_FILENAME) as devices_file:
                    devices = load(devices_file)
            except Exception as e:
                log.exception(e)
        else:
            connected_devices_file = open(self._config_dir + cons.CONNECTED_DEVICES_FILENAME, 'w')
            ...
            for device_name in devices:
                try:
                    if not isinstance(devices[device_name], tuple):
                        open(self._config_dir + cons.CONNECTED_DEVICES_FILENAME, 'w').close()
    ...
    def __save_persistent_devices(self):
        with open(self._config_dir + cons.CONNECTED_DEVICES_FILENAME, 'w') as config_file:
            try:
                data_to_save = {}
                for device in self.__connected_devices:
                    if self.__connected_devices[device]["connector"] is not None:
